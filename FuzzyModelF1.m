...
% Zbiory rozmyte dla wejścia F1
% (Zał: Fd = 100)
F1Range = [50, 500];

NumOfFuzzySets = 4;
F1p = linspace(F1Range(1), F1Range(2), NumOfFuzzySets);
DF1p = F1p(2) - F1p(1);
...
figure(1);
x = linspace(F1Range(1), F1Range(2), 1000);
hold on;
title('Funkcje przynależności dla wejścia F1');
xlabel('F1');
for i = 1:NumOfFuzzySets
    plot(x, mf{i}(x));
end
